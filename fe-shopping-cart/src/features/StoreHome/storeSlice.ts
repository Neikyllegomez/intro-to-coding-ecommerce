...
import { saveInvoice } from '../../api/endpoints';
...
//save store items to api
export const saveStoreItems = createAsyncThunk(
  'store/saveStoreItems',
  async (invoice: InvoicePayload) => {
    const response = await api.post(saveInvoice, invoice)
    return response.data
